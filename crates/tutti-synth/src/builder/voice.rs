...
use super::SvfMode;
use tutti_core::dsp::{
    adsr_live, bandpass_q, dc, highpass_q, lowpass_q, moog_q, notch_q, pass, pink, poly_pulse,
...
impl SynthVoice {
    ...
    fn update_modulated_cutoff(&mut self) {
        let fm = &self.filter_mod;

        if fm.mod_wheel_depth == 0.0 && fm.velocity_depth == 0.0 && fm.lfo_depth == 0.0 {
            return;
        }
        ...
        if fm.velocity_depth > 0.0 {
            let vel_mult = 1.0 - fm.velocity_depth * 0.5 + self.velocity_mod_value * fm.velocity_depth * 0.5;
            cutoff *= vel_mult;
        }
...
fn build_sub_voice_dsp(
    ...
    match (&config.oscillator, &config.filter) {
        ...
        (OscillatorType::Square { pulse_width }, FilterType::None) => {
            Box::new(
                ((var(pitch) | dc(*pulse_width)) >> poly_pulse::<f32>()) * make_env(gate, env),
            )
        }
        ...
        (OscillatorType::Square { pulse_width }, FilterType::Moog { resonance, .. }) => {
            Box::new(
        ...
        (OscillatorType::Sine, FilterType::Svf { q, mode: SvfMode::Lowpass, .. }) => Box::new(
            ((var(pitch) >> sine::<f32>()) | var(filter_cutoff))
                >> lowpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Saw, FilterType::Svf { q, mode: SvfMode::Lowpass, .. }) => Box::new(
            ((var(pitch) >> saw()) | var(filter_cutoff))
                >> lowpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Square { pulse_width }, FilterType::Svf { q, mode: SvfMode::Lowpass, .. }) => {
            Box::new(
                (((var(pitch) | dc(*pulse_width)) >> poly_pulse::<f32>()) | var(filter_cutoff))
                    >> lowpass_q::<f32>(*q)
                    >> (make_env(gate, env) * pass()),
        ...
        (OscillatorType::Triangle, FilterType::Svf { q, mode: SvfMode::Lowpass, .. }) => Box::new(
            ((var(pitch) >> triangle()) | var(filter_cutoff))
                >> lowpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Noise, FilterType::Svf { q, mode: SvfMode::Lowpass, .. }) => Box::new(
            (pink::<f32>() | var(filter_cutoff))
                >> lowpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),

        (OscillatorType::Sine, FilterType::Svf { q, mode: SvfMode::Highpass, .. }) => Box::new(
            ((var(pitch) >> sine::<f32>()) | var(filter_cutoff))
                >> highpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Saw, FilterType::Svf { q, mode: SvfMode::Highpass, .. }) => Box::new(
            ((var(pitch) >> saw()) | var(filter_cutoff))
                >> highpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Square { pulse_width }, FilterType::Svf { q, mode: SvfMode::Highpass, .. }) => {
            Box::new(
                (((var(pitch) | dc(*pulse_width)) >> poly_pulse::<f32>()) | var(filter_cutoff))
                    >> highpass_q::<f32>(*q)
                    >> (make_env(gate, env) * pass()),
        ...
        (OscillatorType::Triangle, FilterType::Svf { q, mode: SvfMode::Highpass, .. }) => Box::new(
            ((var(pitch) >> triangle()) | var(filter_cutoff))
                >> highpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Noise, FilterType::Svf { q, mode: SvfMode::Highpass, .. }) => Box::new(
            (pink::<f32>() | var(filter_cutoff))
                >> highpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),

        (OscillatorType::Sine, FilterType::Svf { q, mode: SvfMode::Bandpass, .. }) => Box::new(
            ((var(pitch) >> sine::<f32>()) | var(filter_cutoff))
                >> bandpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Saw, FilterType::Svf { q, mode: SvfMode::Bandpass, .. }) => Box::new(
            ((var(pitch) >> saw()) | var(filter_cutoff))
                >> bandpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Square { pulse_width }, FilterType::Svf { q, mode: SvfMode::Bandpass, .. }) => {
            Box::new(
                (((var(pitch) | dc(*pulse_width)) >> poly_pulse::<f32>()) | var(filter_cutoff))
                    >> bandpass_q::<f32>(*q)
                    >> (make_env(gate, env) * pass()),
        ...
        (OscillatorType::Triangle, FilterType::Svf { q, mode: SvfMode::Bandpass, .. }) => Box::new(
            ((var(pitch) >> triangle()) | var(filter_cutoff))
                >> bandpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Noise, FilterType::Svf { q, mode: SvfMode::Bandpass, .. }) => Box::new(
            (pink::<f32>() | var(filter_cutoff))
                >> bandpass_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),

        (OscillatorType::Sine, FilterType::Svf { q, mode: SvfMode::Notch, .. }) => Box::new(
            ((var(pitch) >> sine::<f32>()) | var(filter_cutoff))
                >> notch_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Saw, FilterType::Svf { q, mode: SvfMode::Notch, .. }) => Box::new(
            ((var(pitch) >> saw()) | var(filter_cutoff))
                >> notch_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Square { pulse_width }, FilterType::Svf { q, mode: SvfMode::Notch, .. }) => {
            Box::new(
                (((var(pitch) | dc(*pulse_width)) >> poly_pulse::<f32>()) | var(filter_cutoff))
                    >> notch_q::<f32>(*q)
                    >> (make_env(gate, env) * pass()),
        ...
        (OscillatorType::Triangle, FilterType::Svf { q, mode: SvfMode::Notch, .. }) => Box::new(
            ((var(pitch) >> triangle()) | var(filter_cutoff))
                >> notch_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
        (OscillatorType::Noise, FilterType::Svf { q, mode: SvfMode::Notch, .. }) => Box::new(
            (pink::<f32>() | var(filter_cutoff))
                >> notch_q::<f32>(*q)
                >> (make_env(gate, env) * pass()),
        ),
