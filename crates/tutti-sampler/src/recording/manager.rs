...
use crate::recording::{
    RecordedData, RecordingBuffer, RecordingConfig, RecordingMode, RecordingSession,
...
impl RecordingManager {
    ...
    pub fn resize(&self, _new_track_count: usize) {
        // DashMap is sparse and grows dynamically, no need to resize
    }
    ...
    pub fn start_recording(
        ...
        // For audio input recording, setup Butler capture
    ...
    pub fn stop_recording(&self, channel_index: usize) -> crate::error::Result<RecordedData> {
        ...
        let session = self
            ...
            .map(|s| Arc::clone(&s))?;
        // DashMap read lock is released here
        ...
        // Now safe to remove from DashMap since we're not holding the read lock
    ...
    // =========================================================================
    // Preroll Management (called from audio thread)
    // =========================================================================
    ...
    // =========================================================================
    // Event Recording (called from UI thread)
    // =========================================================================
    ...
    // =========================================================================
    // Snapshot Access (for audio thread if needed)
    // =========================================================================
    ...
    // =========================================================================
    // Butler Integration (Audio Input Recording)
    // =========================================================================
    ...
    fn setup_audio_input_capture(&self, session: &RecordingSession) -> crate::error::Result<()> {
        ...
        // Create capture buffer (ring buffer for lock-free audio thread → Butler communication)
        // Default: 1 second buffer (1000ms)
        ...
        // Register capture with Butler thread
        self.butler_tx
        ...
        // Store capture info and producer in session
    ...
    fn stop_audio_input_capture(
        ...
        // Flush capture buffer to disk (urgent priority for immediate stop)
        self.butler_tx
            .send(ButlerCommand::Flush(FlushRequest::urgent(
                capture_id,
                file_path.clone(),
            )))
        ...
        // Read duration from WAV file metadata
        let duration_seconds = match hound::WavReader::open(&file_path) {
            ...
            Err(_) => {
                // Fallback: use 0.0 if file read fails
                0.0
            }
        };
...
mod tests {
    ...
    fn test_resize() {
        let manager = create_test_manager();
        // DashMap grows dynamically, resize is a no-op
        manager.resize(16);
        assert!(!manager.is_recording(0));
    }
    ...
    fn test_concurrent_recording() {
        ...
        // Record events to different tracks
        manager.record_midi_note_on(0, 60, 100, 0.0, 0).unwrap();
        ...
        match data0 {
            RecordedData::Midi { buffer } => {
                assert_eq!(buffer.active_note_count(), 1); // Note still active
            }
            _ => panic!("Expected MIDI data"),
        }
    }
